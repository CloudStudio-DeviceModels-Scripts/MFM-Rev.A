...
//endpoints.addEndpoint("1","voltageSensor",endpointType.voltageSensor);                 
  //endpoints.addEndpoint("2","currentSensor",endpointType.currentSensor);
  //endpoints.addEndpoint("3","activePowerSensor",endpointType.activePowerSensor);


var kalki_meter = payload.asJsonObject();
    env.log(kalki_meter); 
	// Verify payload contains exactly 3 bytes
	//if (bytes.length != 3)
		//return;
	// Parse and store temperature
	var voltage = device.endpoints.byType(endpointType.voltageSensor);
	if (voltage != null)
	{
        //updateVoltageSensorStatus(voltageVolts [, utcDateTime])
		temp.updateVoltageSensorStatus(kalki_meter.voltage);
	}
    var current = device.endpoints.byType(endpointType.currentSensor);
	if (current != null)
	{
		current.updateCurrentSensorStatus(kalki_meter.current);
	}

    var activePowerSensor = device.endpoints.byType(endpointType.activePowerSensor);
	if (activePowerSensor != null)
	{
		current.updateActivePowerSensorStatus(kalki_meter.activePower);
        //updateActivePowerSensorStatus(activePowerWatts [, utcDateTime])
	}
}
